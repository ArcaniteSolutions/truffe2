    ...
    def handle(self, *args, **options):
        ...
        def _do(unit, roles):
            ...
            for accred in unit.accreditation_set.filter(end_date=None, role__pk__in=roles):
                ...
                rlc_accred, created = Accreditation.objects.get_or_create(unit=rlc_unit, user=accred.user, end_date=None, role=rlc_role, display_name=u'%s %s' % (settings.AUTO_RLC_TAG, accred.get_role_or_display_name()), no_epfl_sync=False, hidden_in_epfl=True, hidden_in_truffe=True, need_validation=False)
