...
urlpatterns = patterns(
    ...
    url(r'^password_reset/$', password_reset, {'post_reset_redirect': '/users/login?why=reset_done'}, name='password_reset'),
    url(r'^reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
        password_reset_confirm, {'post_reset_redirect': '/users/login?why=reset_completed'}, name='password_reset_completed'),
