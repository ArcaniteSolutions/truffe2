...
def ldap_search(request):
    ...
    retour += map(lambda x: {'id': x.username, 'text': u'{} - {} {} ({}){}'.format(x.username, x.first_name, x.last_name, x.email, u'(Pas dans l\'annuaire EPFL !)' if x.username_is_sciper() and not retour else u'')}, internal)
