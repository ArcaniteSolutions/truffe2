...
def internaltransfer_csv(request, pk):
    ...
    for transfer in transfers:
        if not transfer.status[0] in ['4', '5', '6']:
            raise Exception(_(u'Internal Transfer {0} pas à l\'état à comptabiliser').format(unicode(transfer)))
...
def expenseclaim_line_write(writer, expenseclaim, line, line_number, last_line, expenseclaim_number):
    ...
    row = [u'1', '', '', '', '', '', '', '', '', '', '', expenseclaim_number * 10000 + line_number, line_number, line.value, u'NDF - {0} - {1}'.unicode(expenseclaim, line.label), line.account.account_number, tva_string, tva.code, tva.value, u'OK', u'NDF#{0}'.format(unicode(expenseclaim.pk)), expenseclaim.costcenter.account_number]
